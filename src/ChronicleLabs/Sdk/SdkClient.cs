...
public partial class SdkClient : ISdkClient
{
    private readonly RawClient _client;

    internal SdkClient(RawClient client)
    {
        _client = client;
    }
    ...
    )
    {
        ...
        if (response.StatusCode is >= 200 and < 400)
        {
            ...
            try
            {
                var responseData = JsonUtils.Deserialize<AcceptedResponse>(responseBody)!;
                return new WithRawResponse<AcceptedResponse>()
                {
                    Data = responseData,
    ...
    /// </code></example>
    public WithRawResponseTask<AcceptedResponse> IdentifyUserAsync(
        IdentifyUserRequest request,
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
    {
        return new WithRawResponseTask<AcceptedResponse>(
            IdentifyUserAsyncCore(request, options, cancellationToken)
        );
    }
    ...
    /// </code></example>
    public WithRawResponseTask<AcceptedResponse> TrackSignalsAsync(
        TrackSignalsRequest request,
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
    {
        return new WithRawResponseTask<AcceptedResponse>(
            TrackSignalsAsyncCore(request, options, cancellationToken)
        );
    }
    ...
    /// </code></example>
    public WithRawResponseTask<AcceptedResponse> TrackTracesAsync(
        TrackTracesRequest request,
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
    {
        return new WithRawResponseTask<AcceptedResponse>(
            TrackTracesAsyncCore(request, options, cancellationToken)
        );
    }
}
